...
static int tostring_s64(lua_State* L) {
  char temp[64];
  sprintf(temp, "%" I64_FMT "d", *reinterpret_cast<const long long*>(lua_topointer(L, 1)));
...
static int tostring_u64(lua_State* L) {
  char temp[64];
  sprintf(temp, "%" I64_FMT "u",
          *(reinterpret_cast<const unsigned long long*>(lua_topointer(L, 1))));
  lua_pushstring(L, temp);
  return 1;
}
...
static int eq_u64(lua_State* L) {
  lua_pushboolean(L, memcmp(lua_topointer(L, 1), lua_topointer(L, 2), sizeof(unsigned long long)) ==
                         0);
  return 1;
}
...
static int lt_u64(lua_State* L) {
  lua_pushboolean(L,
                  memcmp(lua_topointer(L, 1), lua_topointer(L, 2), sizeof(unsigned long long)) < 0);
  return 1;
}
...
static void call_stack(lua_State* L, int n) {
  ...
  if (lua_getstack(L, n, &ar) == 1) {
    ...
    if (ar.name)
      lua_tinker::print_error(L, "%s%s() : line %d [%s : line %d]", indent, ar.name, ar.currentline,
                              ar.source, ar.linedefined);
    else
      lua_tinker::print_error(L, "%sunknown : line %d [%s : line %d]", indent, ar.currentline,
                              ar.source, ar.linedefined);
...
/* read                                                                      */
/*---------------------------------------------------------------------------*/
template <> char* lua_tinker::read(lua_State* L, int index) {
  return (char*)lua_tostring(L, index);
}

template <> const char* lua_tinker::read(lua_State* L, int index) { return lua_tostring(L, index); }

template <> char lua_tinker::read(lua_State* L, int index) {
  return static_cast<char>(lua_tonumber(L, index));
}

template <> unsigned char lua_tinker::read(lua_State* L, int index) {
  return static_cast<unsigned char>(lua_tonumber(L, index));
}

template <> short lua_tinker::read(lua_State* L, int index) {
  return static_cast<short>(lua_tonumber(L, index));
}

template <> unsigned short lua_tinker::read(lua_State* L, int index) {
  return static_cast<unsigned short>(lua_tonumber(L, index));
}

template <> long lua_tinker::read(lua_State* L, int index) {
...
template <> unsigned long lua_tinker::read(lua_State* L, int index) {
  if (lua_isnumber(L, index))
    return static_cast<unsigned long>(lua_tonumber(L, index));
  else
    return *reinterpret_cast<unsigned long*>(lua_touserdata(L, index));
}

template <> int lua_tinker::read(lua_State* L, int index) {
  return static_cast<int>(lua_tonumber(L, index));
}

template <> unsigned int lua_tinker::read(lua_State* L, int index) {
  return static_cast<unsigned int>(lua_tonumber(L, index));
}

template <> float lua_tinker::read(lua_State* L, int index) {
  return static_cast<float>(lua_tonumber(L, index));
}

template <> double lua_tinker::read(lua_State* L, int index) {
  return static_cast<double>(lua_tonumber(L, index));
}

template <> bool lua_tinker::read(lua_State* L, int index) {
...
template <> void lua_tinker::read(lua_State* L, int index) {
  L = L;
  index = index;
  return;
}

template <> long long lua_tinker::read(lua_State* L, int index) {
  if (lua_isnumber(L, index))
    return static_cast<long long>(lua_tonumber(L, index));
  else
    return *reinterpret_cast<long long*>(lua_touserdata(L, index));
}
template <> unsigned long long lua_tinker::read(lua_State* L, int index) {
  if (lua_isnumber(L, index))
    return static_cast<unsigned long long>(lua_tonumber(L, index));
  else
    return *reinterpret_cast<unsigned long long*>(lua_touserdata(L, index));
}

template <> lua_tinker::table lua_tinker::read(lua_State* L, int index) { return table(L, index); }

/*---------------------------------------------------------------------------*/
/* push                                                                      */
/*---------------------------------------------------------------------------*/
template <> void lua_tinker::push(lua_State* L, char ret) { lua_pushnumber(L, ret); }

template <> void lua_tinker::push(lua_State* L, unsigned char ret) { lua_pushnumber(L, ret); }

template <> void lua_tinker::push(lua_State* L, short ret) { lua_pushnumber(L, ret); }

template <> void lua_tinker::push(lua_State* L, unsigned short ret) { lua_pushnumber(L, ret); }

template <> void lua_tinker::push(lua_State* L, long ret) {
  *reinterpret_cast<long long*>(lua_newuserdata(L, sizeof(long long))) = ret;
  lua_pushstring(L, "__s64");
  lua_gettable(L, LUA_GLOBALSINDEX);
  lua_setmetatable(L, -2);
}

template <> void lua_tinker::push(lua_State* L, unsigned long ret) {
  *reinterpret_cast<unsigned long long*>(lua_newuserdata(L, sizeof(unsigned long long))) = ret;
  lua_pushstring(L, "__u64");
  lua_gettable(L, LUA_GLOBALSINDEX);
  lua_setmetatable(L, -2);
}

template <> void lua_tinker::push(lua_State* L, int ret) { lua_pushnumber(L, ret); }

template <> void lua_tinker::push(lua_State* L, unsigned int ret) { lua_pushnumber(L, ret); }

template <> void lua_tinker::push(lua_State* L, float ret) { lua_pushnumber(L, ret); }

template <> void lua_tinker::push(lua_State* L, double ret) { lua_pushnumber(L, ret); }

template <> void lua_tinker::push(lua_State* L, char* ret) { lua_pushstring(L, ret); }

template <> void lua_tinker::push(lua_State* L, const char* ret) { lua_pushstring(L, ret); }

template <> void lua_tinker::push(lua_State* L, bool ret) { lua_pushboolean(L, ret); }

template <> void lua_tinker::push(lua_State* L, lua_value* ret) {
  if (ret)
    ret->to_lua(L);
  else
    lua_pushnil(L);
...
template <> void lua_tinker::push(lua_State* L, long long ret) {
  *reinterpret_cast<long long*>(lua_newuserdata(L, sizeof(long long))) = ret;
  lua_pushstring(L, "__s64");
  lua_gettable(L, LUA_GLOBALSINDEX);
  lua_setmetatable(L, -2);
}
template <> void lua_tinker::push(lua_State* L, unsigned long long ret) {
  *reinterpret_cast<unsigned long long*>(lua_newuserdata(L, sizeof(unsigned long long))) = ret;
  lua_pushstring(L, "__u64");
  lua_gettable(L, LUA_GLOBALSINDEX);
  lua_setmetatable(L, -2);
...
template <> void lua_tinker::push(lua_State* L, lua_tinker::table ret) {
...
/* pop                                                                       */
/*---------------------------------------------------------------------------*/
template <> void lua_tinker::pop(lua_State* L) { lua_pop(L, 1); }

template <> lua_tinker::table lua_tinker::pop(lua_State* L) { return table(L, lua_gettop(L)); }
